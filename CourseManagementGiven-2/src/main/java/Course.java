...
public class Course {
    ...
    If collection size is only of size 2,
    it will take both values and get the average.
    For anything of size greater than 2 it will,
    sum the inputs from the array and return the average.
    ...
    //SER316 TASK 2 SPOT-BUGS FIX
    //Removed int, was never used
    
    //SER316-start, must not have students with the same name  
    public boolean addStudent(Student s) {
        String asurite = s.getAsurite();
        if (Arrays.asList(students).contains(asurite)) {
            return false;
        }
        else {
            students.add(s);
            points.put(asurite, -1);
            return true;
        }
    }
